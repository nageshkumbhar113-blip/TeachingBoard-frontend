...
const TEST_PLAYER = (() => {
  ...
  async function startTest(quiz_id, mode = 'practice') {
    ...
    const hasQ = (quiz.sections || []).some(s => s.question_ids?.length);
    if (!hasQ)  { APP.toast('This quiz has no questions yet', 'error'); return; }

    let questions = await _loadQuizQuestions(quiz);

    // If no questions in IDB and we're online, pull from server then retry once
    if (!questions.length && navigator.onLine) {
      APP.toast('Syncing questions…', 'info');
      await API.syncStudentQuestions().catch(() => {});
      questions = await _loadQuizQuestions(quiz);
    }
  ...
  async function _loadQuizQuestions(quiz) {
    ...
    // Merge embedded questions (backend/remote quizzes) so IDB standalone store
    // doesn't need to have them separately
    const embedded = (quiz.questions || []).filter(q => q.q_id);
    const qMap = Object.fromEntries([
      ...allQs.map(q => [q.q_id, q]),
      ...embedded.map(q => [q.q_id, q]),
    ]);

    const flat = [];
    (quiz.sections || []).forEach((sec, sIdx) => {
      const posMarks = sec.positive_marks ?? quiz.positive_marks ?? 1;
      const negMarks = sec.negative_marks ?? quiz.negative_marks ?? 0;
      const secTimer = sec.timer          ?? quiz.timer_value    ?? 30;

      (sec.question_ids || []).forEach(q_id => {
        const q = qMap[q_id];
        if (!q) return;
        flat.push({
          ...q,
          _secIdx  : sIdx,
          _secLabel: sec.label || `Section ${sIdx + 1}`,
          _secType : sec.type  || q.type || 'mcq',
          _posMarks: posMarks,
          _negMarks: negMarks,
          _secTimer: secTimer,
        });
  ...
  function _recordAnswer(q, given, isCorrect) {
    ...
    if (isCorrect) DB.incrementCorrect(q.q_id).catch(() => {});
    else           DB.incrementWrong(q.q_id).catch(() => {});
  }
  ...
  function _startPerQTimer(seconds) {
    ...
    state.perQInterval = setInterval(() => {
      state.perQRemaining--;
      if (count) count.textContent = state.perQRemaining;
      if (ring) {
  ...
  async function _submitTest() {
    ...
    SYNC.submitAttempt(attempt).catch(() => {});

    _showResults(attempt, qs);
  }
  ...
  function _showResults(attempt, questions) {
    ...
    $('tp-btn-weak').onclick  = async () => {
      ...
      const saves = weakAnswers
        .map(a => qMap.get(a.q_id))
        .filter(Boolean)
        .map(q => DB.saveQuestion({ ...q, flagged: true }).catch(() => {}));
