...
const ADMIN = (() => {
  ...
  async function _checkPin() {
    ...
    if (navigator.onLine) {
      ...
      // offline fallback: compare against last cached pin
  ...
  async function _loadAdminContent() {
    API.setApiUrl(await DB.getSetting('api_url', API.DEFAULT_API_URL));
    await Promise.all([
      _loadBatchOptions(),
      loadQuestionBank(),
      _loadLessonAdmin(),
  ...
  async function _loadBatchOptions() {
    ...
    const selects = [
      $('admin-batch-filter'), $('import-batch'), $('bulk-batch'), $('qe-batch'),
    ];
    selects.forEach(sel => {
      if (!sel) return;
  ...
  async function loadQuestionBank() {
    ...
    const subject = $('admin-subject-filter')?.value || '';
    const chapter = $('admin-chapter-filter')?.value || '';
    ...
    if (!filtered.length) {
      list.innerHTML = '<p style="color:var(--text2);font-size:0.85rem;padding:10px">No questions found</p>';
      return;
    }
  ...
  function _downloadSampleCSV() {
    const rows = [
      ...
      // MCQ example
      '"What is the capital of France?",Paris,London,Berlin,Rome,A,,medium,mcq',

      // Another MCQ
      '"Which planet is closest to the Sun?",Mercury,Venus,Earth,Mars,A,,easy,mcq',

      // True/False — A=True B=False; C & D left empty
      '"The Great Wall of China is visible from space.",True,False,,,B,,medium,tf',

      // Fill in the blank — correct answer in column A; B/C/D left empty
      '"Photosynthesis takes place in the ___ of plant cells.",Chloroplast,Mitochondria,Nucleus,Cytoplasm,A,,hard,fib',

      // Hard MCQ
      '"Newton\'s Second Law states that Force equals mass times ___.",Acceleration,Velocity,Weight,Momentum,A,,hard,mcq',
    ];
    const csv  = rows.join('\r\n');
  ...
  // ════════════════════════
  // BULK PASTE (ChatGPT format)
  // ════════════════════════

  function _parseBulkPaste(text) {
    const questions = [];
    const errors    = [];

    // Split on blank lines or on "Q<n>." / "<n>." at the start of a line
    const blocks = text
      .replace(/\r\n/g, '\n')
      .split(/\n{2,}/)
      .map(b => b.trim())
    ...
    blocks.forEach((block, idx) => {
      ...
      // ── Question line ──────────────────────────────────────
      // Accept:  "Q1. text"  "1. text"  "1) text"  or plain text
      let questionText = lines[0]
        .replace(/^Q?\d+[\.\)]\s*/i, '')
        .trim();

      if (!questionText) { errors.push(`Block ${idx+1}: no question text`); return; }
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        // Option lines: "A) text"  "A. text"  "(A) text"  "A - text"
        const optMatch = line.match(/^\(?([A-Da-d])\)?[\.\)\-\s]+(.+)/);
        if (optMatch) {
          optMap[optMatch[1].toUpperCase()] = optMatch[2].trim();
          continue;
        }

        // Answer: A  /  Correct Answer: B  /  Ans: C
        const ansMatch = line.match(/^(?:correct\s+)?ans(?:wer)?\s*:\s*([A-Da-d])/i);
        if (ansMatch) { answerLetter = ansMatch[1].toUpperCase(); continue; }

        // Difficulty: easy / medium / hard
        const diffMatch = line.match(/^difficulty\s*:\s*(\w+)/i);
        if (diffMatch) {
        ...
        if (typeMatch) {
          ...
          if (['mcq','tf','fib','mtp'].includes(t)) type = t;
          continue;
        }
      }
      ...
      // ── Defaults per type ──────────────────────────────────
      if (type === 'tf') {
        if (!optMap['A']) optMap['A'] = 'True';
        if (!optMap['B']) optMap['B'] = 'False';
        if (!answerLetter) answerLetter = 'A';
      }

      if (!answerLetter) answerLetter = 'A';

      // ── Validate ───────────────────────────────────────────
      if (type === 'mcq' && (!optMap['A'] || !optMap['B'])) {
        errors.push(`Block ${idx+1}: MCQ needs at least A and B options`);
        return;
      }
  ...
  async function _importBulkPaste() {
    const text    = ($('bulk-paste-input')?.value || '').trim();
    const batch   = $('bulk-batch')?.value;
    const subject = ($('bulk-subject')?.value || '').trim();
    const chapter = ($('bulk-chapter')?.value || '').trim();
    const log     = $('bulk-import-log');

    if (!text)    { APP.toast('Paste kahi text nahi', 'error'); return; }
    if (!batch || !subject || !chapter) {
      APP.toast('Class, Subject, Chapter fill kara', 'error'); return;
    }

    log.innerHTML = 'Parsing…'; log.classList.add('visible');

    const { questions, errors } = _parseBulkPaste(text);

    if (!questions.length) {
      log.innerHTML = `<span class="log-error">❌ Koi valid question sapadli nahi\n${errors.join('\n')}</span>`;
      return;
    }

    const toSave = questions.map(q => ({ ...q, batch, subject, chapter }));

    try {
      await DB.saveQuestionsBatch(toSave);
      let html = `<span class="log-success">✅ Import jhale: ${toSave.length} questions</span>\n`;
      if (errors.length) html += `<span class="log-warn">⚠️ Skip jhale:\n${errors.join('\n')}</span>`;
      log.innerHTML = html;
      APP.toast(`✅ ${toSave.length} questions import jhale`, 'success');
      $('bulk-paste-input').value = '';
      await loadQuestionBank();
      APP.refreshHome();
    } catch (err) {
      log.innerHTML = `<span class="log-error">❌ Save failed: ${err.message}</span>`;
    }
  }
  ...
  async function _importZIP() {
    ...
    if (typeof JSZip === 'undefined') {
      APP.toast('JSZip library required', 'error'); return;
    }
  ...
  function _renderQuizGroup(containerId, quizzes) {
    ...
    quizzes.forEach(quiz => {
      ...
      item.querySelector('.quiz-play-btn')?.addEventListener('click', () => {
        close();
        TEST_PLAYER.startTest(quiz.quiz_id, 'practice');
      });
  ...
  function init() {
    ...
    ['admin-batch-filter','admin-subject-filter','admin-chapter-filter'].forEach(id => {
      $(id)?.addEventListener('change', loadQuestionBank);
    });
    ...
    $('btn-copy-prompt')?.addEventListener('click', () => {
      ...
      navigator.clipboard.writeText(text).then(
        ...
        () => APP.toast('Copy failed — manually select & copy', 'error')
      );
    });
    $('btn-bulk-import')?.addEventListener('click', _importBulkPaste);
