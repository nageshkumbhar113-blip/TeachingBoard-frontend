...
const NOTES_PDF = (() => {
  ...
  async function _renderToBlob(html, chrome = {}) {
    ...
    try {
      ...
      // Real bug found live (core/exercisePdf.js): description-block <img>s
      // (Cloudinary-hosted, real network latency) can still be mid-download
      // when html2canvas snapshots the container — the font-ready wait above
